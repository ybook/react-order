...
react + react-router + redux + ES6 + webpack + webpack-dev-server + styled-components
...
**涉及的一些问题：**
1.之前用vue写习惯一个组件里面集合css，html，js，于是用react想继续沿用vue单组件html，css，js集合，这里推荐使用两种方式

(1)[css-modules](http://www.ruanyifeng.com/blog/2016/06/css_modules.html) 推荐参考阮一峰老师的介绍，比较详细通俗易懂

(2)styled-components 这个应该算最新的样式组件化插件了，使用起来非常方便

```js
//cnpm安装插件
cnpm install styled-components --save-dev
...
字符串拼接内写样式，只会作用于这个Div组建里面，首字母一定要大写

可以使用伪类选择器，伪元素选择器，动画，类似sass嵌套写法
[具体使用参考点击跳转](https://juejin.im/entry/59a57a2b5188252445327ac1)

2.关于react项目中className多个的问题

使用字符串模版拼接解决
```js
<div className={`ordertext${this.props.orderonline?'':' hidde'}`}></div>
```

**react中必须要了解的知识**
(以下知识仅供参考,转载于他人,建议参考官方文档，没个人理解不同，尽量使用比较容易理解的方式去使用框架)
比如：redux,flux

字面去理解有时候很难理解，特别是刚开始学的时候

其实可以最简单理解为变量作用域（底层并非这样，便于理解而已）

store就相当于一个全局变量，组件想使用链接直接connect后就可以使用，要修改的话就需要dispatch和action结合操作

而每个组件中的state可以理解为你这个组建的局部变量

然后根据不同的情况去处理这些数据实现数据互通
